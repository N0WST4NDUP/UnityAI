...
- **한 Phase가 여러 날에 걸치면 날짜별로 항목을 계속 추가(append)** — 같은 Phase 내 세션이라도 새 항목으로 기록

### 3. weak-points.md 업데이트 ⚠️ 절대 빠뜨리지 말 것
- 세션 중 막혔거나 재질문이 발생한 개념 기록
- 해결된 경우 상태를 `해결`로 표기
- **기록할 항목이 없더라도 반드시 확인하고 "없음" 명시** — 이 항목을 건너뛰지 않는다
...
> **주의:** git push는 학습자가 명시적으로 요청할 때만 실행한다. 세션 종료 시 자동 push 금지.
